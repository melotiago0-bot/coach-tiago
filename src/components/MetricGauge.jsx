export default function MetricGauge({ label, value, unit, segments, badge, badgeColor, context }) {
  if (!value) return (
  ...
  return (
    <div className="metric-card">
      ...
      <div style={{ height: '5px', borderRadius: '4px', overflow: 'hidden', display: 'flex', marginBottom: '3px' }}>
        {segments.map((seg, i) => (
          <div key={i} style={{ flex: seg.width, background: seg.color }} />
        ))}
      </div>
...
export const HRV_CONFIG = (value) => {
  ...
  return {
    segments: [
      { min: 0, max: 30, width: 20, color: '#F09595' },
      { min: 30, max: 40, width: 15, color: '#EF9F27' },
      { min: 40, max: 55, width: 30, color: '#5DCAA5' },
      { min: 55, max: 100, width: 35, color: '#1D9E75' },
    ],
    badge,
    badgeColor,
    context: '<30 baixo · 30-40 normal · 40-55 bom · >55 ótimo',
  };
};
...
export const RHR_CONFIG = (value) => {
  ...
  return {
    segments: [
      { min: 30, max: 55, width: 35, color: '#1D9E75' },
      { min: 55, max: 65, width: 25, color: '#5DCAA5' },
      { min: 65, max: 75, width: 25, color: '#EF9F27' },
      { min: 75, max: 100, width: 15, color: '#F09595' },
    ],
    badge,
    badgeColor,
    context: '<55 atleta · 55-65 bom · 65-75 normal · >75 alto',
  };
};
...
export const SLEEP_CONFIG = (value) => {
  ...
  return {
    segments: [
      { min: 0, max: 5, width: 20, color: '#F09595' },
      { min: 5, max: 6, width: 15, color: '#EF9F27' },
      { min: 6, max: 7, width: 25, color: '#FAC775' },
      { min: 7, max: 10, width: 40, color: '#1D9E75' },
    ],
    badge,
    badgeColor,
    context: '<5h insuficiente · 5-6h baixo · 6-7h moderado · >7h ótimo',
  };
};
